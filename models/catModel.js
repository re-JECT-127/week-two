...
const insertCat = async (cat) => {
  try {
    ...
    const [rows] = await promisePool.query(
        'INSERT INTO wop_cat (name, age, weight, owner, filename) VALUES (?, ?, ?, ?, ?)', [ cat.name, cat.age, cat.weight, cat.owner, cat.filename ]);
    return rows;
  } catch (e) {
    console.error('error', e.message);
  }
...
const updateCat = async (cat) => {
  try {
    console.log('insert cat?', cat);
    const [rows] = await promisePool.query(
        'UPDATE wop_cat SET name = ?, age = ?, weight = ?, owner = ? WHERE wop_cat.cat_id = ?', [ cat.name, cat.age, cat.weight, cat.owner, cat.id ]);
    return rows;
  } catch (e) {
...
const deleteCat = async (id) => {
  try {
    ...
    const [rows] = await promisePool.query(
        'DELETE FROM wop_cat WHERE wop_cat.cat_id = ?', [ id ]);
